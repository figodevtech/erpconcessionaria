...
export function VehicleTable({ vehicles, loading, showImages }: { vehicles: Vehicle[], loading: boolean, showImages: boolean }) {
    ...
    <div className="w-full relative min-h-[400px] p-4">
      <Table className="text-xs">
        ...
        <TableBody>
          {vehicles.length === 0 ? (
            <TableRow>
              <TableCell colSpan={showImages ? 9 : 8} className="h-24 text-center text-muted-foreground">
                Nenhum veículo encontrado para a busca.
              </TableCell>
            </TableRow>
          ) : (
            vehicles.map((vehicle) => (
              <TableRow className="cursor-pointer h-14" key={vehicle.id}>
                {showImages && <TableCell>
                  <Image src={vehicle.image || ""} alt={vehicle.brand} width={50} height={50} className="rounded-md" />
                </TableCell>}
                <TableCell>
                  {vehicle.id}
                </TableCell>
                ...
                <TableCell className="uppercase text-center">{vehicle.plate || "N/A"}</TableCell>
                <TableCell className="text-center">{formatCurrency(vehicle.price)}</TableCell>
                <TableCell className="text-center">
                  <Badge variant="outline" className={`font-normal border-transparent ${getStatusColor(vehicle.status || '')}`}>
                ...
                <TableCell className="text-center">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="center" className="text-nowrap">
                      <DropdownMenuItem
                      ...
                      <DropdownMenuItem
                        ...
                        className="hover:cursor-pointer bg-blue-600/10 hover:bg-blue-600/20 data-highlighted:bg-blue-600/50 transition-all text-nowrap"
                      >
                        <ArrowLeftRight />
                        Transferir
                      </DropdownMenuItem>
